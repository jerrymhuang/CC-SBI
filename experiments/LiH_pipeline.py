...
from simulations.benchmarks.lih import lih
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="LiH training pipeline for BayesFlow")
...
def main():
    ...
    dm_workflow = bf.workflows.BasicWorkflow(
        ...
        checkpoint_filepath=checkpoint_dir / "lih_diffusion.ckpt",
    )

    # Generate and verify datasets, train, and visualize diagnostics
    try:
        train_set = generate_dataset(
            simulator, args.train_samples, args.num_molecules, out_dir / f"lih_{args.num_molecules}_train.npz"
        )
        val_set = generate_dataset(
            simulator, args.val_samples, args.num_molecules, out_dir / f"lih_{args.num_molecules}_val.npz"
        )
        logging.info("Verifying dataset structure...")
        train_data = load_npz_dict(out_dir / f"lih_{args.num_molecules}_train.npz")
        ...
        model_path = checkpoint_dir / "lih_diffusion.ckpt" / "model.keras"
        ...
        for plot_name, fig in figures.items():
            fig_path = figures_dir / f"lih_{args.num_molecules}_{plot_name}.png"
