...
def main():
    ...
    dm_workflow = bf.workflows.BasicWorkflow(
        ...
        checkpoint_filepath=checkpoint_dir / "ethene_diffusion.ckpt",
    )

    # Generate and verify datasets, train, and visualize diagnostics
    try:
        train_set = generate_dataset(
            simulator, args.train_samples, args.num_molecules, out_dir / f"ethene_{args.num_molecules}_cc-pVDZ_train.npz"
        )
        val_set = generate_dataset(
            simulator, args.val_samples, args.num_molecules, out_dir / f"ethene_{args.num_molecules}_cc-pVDZ_val.npz"
        )
        logging.info("Verifying dataset structure...")
        train_data = load_npz_dict(out_dir / f"ethene_{args.num_molecules}_cc-pVDZ_train.npz")
        ...
        model_path = checkpoint_dir / "ethene_diffusion.ckpt" / "model.keras"
        ...
        for plot_name, fig in figures.items():
            fig_path = figures_dir / f"ethene_{args.num_molecules}_cc-pVDZ_{plot_name}.png"
